...
### Avancement backend estimé : **78 %**

Le socle est solide et bien structuré. La majorité des entités, des routes CRUD, du système d'authentification et du backoffice sont en place. Les parties encore manquantes concernent principalement l'intégration réelle de la passerelle de paiement, la génération de PDF pour les factures, les emails transactionnels (confirmation commande…), et le 2FA admin.
...
| **Authentification** | JWT stateless (LexikJWTAuthenticationBundle 3.2) |
...
| **Génération du QR code TOTP** | ❌ Manquant | Aucun service ni endpoint pour activer le 2FA |
| **Vérification du code TOTP à la connexion** | ❌ Manquant | Le login JWT ne vérifie pas le code TOTP même si `totpEnabled=true` |
...
Installer `scheb/2fa-bundle` (`composer require scheb/2fa-bundle scheb/2fa-totp`). Ajouter un endpoint `POST /api/admin/2fa/enable` (génère un secret TOTP, retourne un QR code en Base64) et `POST /api/admin/2fa/verify`. Modifier le firewall pour exiger le code TOTP quand `totpEnabled=true`.
...
```
Authentification        ████████████░░  85%  (manque 2FA opérationnel + "se souvenir de moi")
